...
\end{align}
The constant $E_0$ is such that the units of $E^*E$ are $\si{\watt\metre^{-2}}$. Mode matching depends only on the shape of the modes, denoted with $\Psi$, so the units of the electric field not feature in this calculation.
\par
For a beam without astigmatism, $x^2 + y^2 = r^2$ and $q_x = q_y = q$, so the shape of the mode can be expressed as
\begin{equation}
\Psi(r,z) = \frac{1}{q(z)}\exp\left(-ik\frac{r^2}{2q(z)}\right).
\end{equation}
The mode matching, $\mathcal{M}$, of two beams is determined by the overlap of the shapes their fields. This can be calculated with
\begin{equation}
